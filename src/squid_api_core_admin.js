                    ...
                    if (response.suggestions && response.suggestions.length > 0) {
                        ...
                        me.$el.siblings(".squid-api-pre-suggestions").find("li").click(me, function(event) {
                            ...
                            if ($(event.target).is("li")) {
                                item = $(event.target).data("suggestion-value");//$(event.target).find(".suggestion").html();
                            } else {
                                item = $(event.target).closest("li").data("suggestion-value");//$(event.target).parent().find(".suggestion").html();
                            }
                            var value = me.$el.val();
